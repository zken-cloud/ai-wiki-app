...
flowchart TD
    ...
    E --> G["select()<br/>min_score · max_items<br/>max_curated · max_arxiv"]
    F --> G
...
- **One item, one category, per run.** The ledger is only written after every
  category has run, so during the loop each category sees a pre-run ledger.
  `build_day()` therefore keeps an in-run `claimed` set — without it, arXiv
  papers cross-listed to cs.CR and cs.LG were collected by `security` and
  `papers` and published twice.
...
| `arxiv` | arXiv Atom API, per category | **Rate-limited to 1 req / 8s**, 6 retries, honours `Retry-After`. 3s was not enough: a multi-week backfill 429s partway and whole days silently lose their paper coverage. Date ranges must go through `params=` so brackets are encoded |
...
│   ├── signal/<date>.md      #   archived one-screen briefs
│   ├── feed.xml              #   Atom: the curated Signal
│   ├── <category>/feed.xml   #   Atom: one per category
...
Feeds are Atom 1.0, generated by `pipeline/feeds.py` from the same JSON the
pipeline already writes — no extra dependency and no model call. Entry
`<updated>` is the date the wiki surfaced the item, not the item's own
publication date, which is carried separately as `<published>`: Hugging Face
features papers weeks after submission, and ordering by `published` pushed
half of the newest content below the entry cap. `/feed.xml`
carries the curated Signal picks; `/<category>/feed.xml` carries everything
published in that category. Feed-level `<updated>` is derived from the newest
entry rather than the wall clock, so a run that publishes nothing regenerates
byte-identical files and produces no commit.
...
| Stop one source type dominating a category | Set `filter.max_arxiv` (reserves the remaining `max_items` slots for non-arXiv sources; unused slots fall back) |
